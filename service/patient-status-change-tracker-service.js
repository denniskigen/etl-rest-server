...
export class PatientStatusChangeTrackerService {
  ...
  getCumulativeAggregateAnalysis(reportParams) {
    return new Promise((resolve, reject) => {
      Promise.all([this.getCumulativeAggregates(reportParams)])
        .then((data) => {
  ...
  getCohortAggregateAnalysis(reportParams) {
    return new Promise((resolve, reject) => {
      Promise.all([this.getMonthlyChangeAggregates(reportParams.requestParams)])
        .then((data) => {
          resolve(data[0]);
        })
        .catch((e) => {
    ...
    _.each(cumulativeAggr.result, (cumulativeRow, i) => {
      ...
      _.each(indicatorsKeys, (key, j) => {
        _.each(indicatorsKeys, (key2, i) => {
          if (!cumulativeRow[key + '_to_' + key2])
            cumulativeRow[key + '_to_' + key2] = 0;
        });
      });
    });
  ...
  getPatientListReport(reportParams) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (underscore.contains(derivedIndicators, reportParams.indicator)) {
        let start = new Date();
        dao.getPatientCareStatusPatientListQuery(reportParams, (results) => {
          if (results.error) {
        ...
        let report = new PatientlistMysqlReport(
          'patintChangeStatusTrackerAggregate',
          reportParams
        );
        try {
          Promise.join(
  ...
  getMonthlyChangeAggregates(reportParams) {
    return new Promise((resolve, reject) => {
      ...
      let start = new Date();
      dao.getPatientCareStatusAggregatesQuery(reportParams, (results) => {
        if (results.error) {
          reject(results);
        } else {
          results.duration = (new Date() - start) / 1000;
