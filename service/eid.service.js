...
function getPatientIdentifiersFromEIDResults(startDate, endDate) {
	return new Promise(function (resolve, reject) {
		definition
			.getAllEidResultsFromAllSites(startDate, endDate)
...
function getPatientIdentifiersWithEidResultsByType(
	host,
	apikey,
	startDate,
	endDate,
	...
	return new Promise(function (resolve, reject) {
		definition
			...
			.then(function (labResults) {
				...
				patientService
					.getPatientUuidsByIdentifiers(ids)
...
function _extractPatientIdentifiersFromResults(labResults) {
	...
	_.each(labResults, function (result) {
		...
		if (result.cd4) allResults = allResults.concat(result.cd4.posts);

		if (result.dnaPcr) allResults = allResults.concat(result.dnaPcr.posts);
	});
...
function getAllEidResultsFromAllSites(startDate, endDate) {
	return new Promise(function (resolve, reject) {
		definition
			.getAvailableLabServers()
			.then(function (servers) {
				var resultsByLab = [];
				Promise.reduce(
					servers,
					function (previousServer, server) {
						return new Promise(function (resolve, reject) {
							definition
								.getAllEidResults(server, startDate, endDate)
								.then(function (response) {
...
function getAllEidResults(server, startDate, endDate) {
	...
	return new Promise(function (resolve, reject) {
		_getEidResultsPerTypeDisregardingErrors(
			server.host,
			server.generalApiKey,
			startDate,
			endDate,
			...
			.then(function (viralLoad) {
				...
				return _getEidResultsPerTypeDisregardingErrors(
					server.host,
					server.cd4ApiKey,
					startDate,
					endDate,
...
function _getEidResultsPerTypeDisregardingErrors(
	host,
	apikey,
	startDate,
	endDate,
...
function getAllEidResultsByType(
	host,
	apikey,
	startDate,
	endDate,
...
function _getAllEidResultsByType(
	host,
	apiKey,
	startDate,
	endDate,
...
function _increamentPageAndGetAllEidResultsByType(
	host,
	apiKey,
	startDate,
	endDate,
...
function getSynchronizedPatientLabResults(patientUuId, locations) {
	...
	return listReachableServers(locations).then(function (reachable) {
...
function _getSynchronizedPatientLabResults(server, patientUuId) {
	...
	var table =
    eidResultsSchema.patientLabResultsSchema.table.schema +
    '.' +
	...
	return obsService
		...
		.then(function (obsResponse) {
			var missingResult = comparison.findAllMissingEidResults(
				mergedEidResults,
				obsResponse
			);
			var conflicts = comparison.findConflictingEidAmrsViralLoadResults(
				mergedEidResults.viralLoad,
				obsResponse
			);
		...
		.catch(function (err) {
			return new Promise(function (resolve, reject) {
				...
				if (err_msg.indexOf('Object with given uuid doesn\'t exist') != -1)
					...
					err_msg.indexOf('Unable to convert object into response content') !=
          -1
				)
...
function getTimedPendingEIDTestResultsByPatientIdentifiers(
	patientIdentifiers,
	referenceDate,
	server
) {
	return new Promise(function (resolveTestResults, rejectTestResults) {
		...
		function abortOnLongWait() {
			...
			if (counter == 1) {
				getPendingEIDTestResultsByPatientIdentifiers(
					patientIdentifiers,
					referenceDate,
					server
				)
...
function getPendingEIDTestResultsByPatientIdentifiers(
	patientIdentifiers,
	referenceDate,
	server
) {
	...
	var logAndGetFilteredResults = function (results) {
		...
		_.each(results, function (row) {
			...
			if (row && row.SampleStatus) {
				if (
					['Completed', 'Rejected', 'Complete'].indexOf(row.SampleStatus) != -1
				) {
					complete.push(row);
				}
		...
		let completeByDay =
      complete.length > 0
      	? _.uniq(complete, (result) => {
      		return result.DateCollected;
      	})
      	: [];
		let inProcessByDay =
      inprocess.length > 0
      	? _.uniq(inprocess, (result) => {
      		return result.DateCollected;
      	})
      	: [];
	...
	return new Promise(function (resolve, reject) {
		getViralLoadTestResultsByPatientIdentifier(
			...
			.then(function (response) {
				...
				} else pending[location_name].viralLoadErrorMsg = response;

				return getPcrTestResultsByPatientIdentifier(
					patientIdentifiers,
					conf.host,
					conf.generalApiKey
				);
			...
			.then(function (response) {
				...
				} else pending[location_name].pcrErrorMsg = response;

				if (conf.loadCd4)
					return getCd4TestResultsByPatientIdentifier(
						patientIdentifiers,
						conf.host,
						conf.cd4ApiKey
					);
			...
			.then(function (response) {
				...
				} else pending[location_name].cd4ErrorMsg = response;
		...
		getViralLoadTestResultsByPatientIdentifier(
				...
				if (response.posts instanceof Array) {
					_.each(response.posts, function (row) {
						...
						if (
							row &&
              row.SampleStatus &&
              ['Completed', 'Rejected', 'Complete'].indexOf(row.SampleStatus) !=
                -1
						) {
							results.viralLoad.push(row);
						}
					});
				} else results[location_name].viralLoadErrorMsg = response;
			...
			.then(function (response) {
              ...
              ['Completed', 'Rejected', 'Complete'].indexOf(row.SampleStatus) !=
				...
				} else results[location_name].pcrErrorMsg = response;
			...
			.then(function (response) {
              ...
              ['Completed', 'Rejected', 'Complete'].indexOf(row.SampleStatus) !=
				...
				} else results[location_name].cd4ErrorMsg = response;
...
function getViralLoadTestResultsByPatientIdentifier(
	patientIdentifier,
	host,
	key
) {
...
function getCd4TestResultsByPatientIdentifier(patientIdentifier, host, key) {
	var startDate = moment(new Date('2004-01-01'))
		.startOf('day')
