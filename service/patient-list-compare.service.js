		...
		getAllResultsForHandler(requestObject, handler)
			.then(function (response) {
				...
				if (compared.summaryStats.totalIncoming !== 0)
					compared.summaryStats.pocIncomingRatio =
            compared.summaryStats.totalPoc /
            compared.summaryStats.totalIncoming;
...
function generateComparisonStats(comparisonResults, incoming, poc) {
	...
	comparisonResults.summaryStats.totalOnlyPoc =
    comparisonResults.onlyPoc.length;
	comparisonResults.summaryStats.totalOnlyIncoming =
    comparisonResults.onlyIncoming.length;
}
...
function resolvePersonIds(personIdsArray) {
	...
	var sql =
    'select t1.person_id, t1.uuid as patient_uuid, t4.patient_id, t1.gender,' +
