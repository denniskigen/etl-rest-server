...
function resolveProgramVisitTypeEncounterUuidsParamsToIds(request) {
	...
	var resolvedPrograms = [];
	var resolvedVisitTypes = [];
	var resolvedEncounterTypes = [];
	var resolveCount = 0;
	...
	if (typeof request.department !== 'undefined') {
		...
		var programTypeUuid = departmentService.getDepartmentProgramUuids(
			department
		);
	}
	...
	return new Promise(function (resolve, reject) {
		...
		getProgramVisitEncounterUUidIdMap()
			.then((result) => {
				...
				if (
					programDefined !== 'undefined' &&
          visittypeDefined === 'undefined' &&
          encounterDefined === 'undefined'
				) {
					console.log(
						'programTypeUuid.length > 0 && visitTypeUuid.length === 0 && encounterTypeUuid.length === 0'
					);
					_.each(programsConfig, (program, index) => {
						if (_.includes(programTypeUuid, index) === true) {
							...
							if (typeof programId !== 'undefined') {
								...
								console.error(
									'ERROR : Undefined ProgramType uuid',
									programUUid
								);
							}
						}
							...
							_.each(visitTypes, (visitType) => {
								...
								if (_.includes(visitTypeUuid, visitUuid) === true) {
									...
									if (typeof visitId !== 'undefined') {
										...
										console.error(
											'ERROR : Undefined VisitType uuid',
											visitUuid
										);
									}
								}
									...
									_.each(encounterTypes, (encounterType) => {
										...
										if (_.includes(encounterTypeUuid, encounterUuid) === true) {
											...
											if (typeof encounterId !== 'undefined') {
												...
												console.error(
													'ERROR:Undefined EncounterType uuid',
													encounterUuid
												);
											}
										}
