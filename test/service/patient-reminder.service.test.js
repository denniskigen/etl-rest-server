...
describe('Patient Reminder Service Unit Tests', function () {
	...
	it('should load  patient reminder module', function () {
		expect(patientReminder).to.be.defined;
	});

	it('should preprocess patient reminder results', function () {
		var initialResult = {
			startIndex: 0,
			size: 1,
			result: [
		...
		var processedResults = {
			...
			result: {
				...
				reminders: [
					{
						message:
              'Viral load test that was ordered on: (19-07-2016)' +
              ' resulted to an error. Please re-order.',
					...
					{
						message:
              'Patient requires viral load. Patients older than 25 years and on ART > 1 year' +
		...
		var processedReminder = patientReminder.generateReminders(
	...
	it('should return pending lab order when it exists in EID', function () {
		var initialResult = {
			startIndex: 0,
			size: 1,
			result: [
		...
		var processedResults = {
			...
			result: {
				...
				reminders: [
					{
						message:
              'Patient lab Order No. is currently being processed. Sample collected' +
              ' on 15-05-2017).',
