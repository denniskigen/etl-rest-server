...
module.exports = (function () {
	return {
		...
		getClinicHivSummayIndicators: function getClinicHivSummayIndicators(
			request,
			callback
		) {
			var uuid = request.params.uuid;
		...
		getClinicAppointmentSchedule: function getClinicAppointmentSchedule(
			request,
			callback
		) {
			var uuid = request.params.uuid;
		...
		getClinicMonthlyAppointmentSchedule: function getClinicMonthlyAppointmentSchedule(
			request,
			callback
		) {
			var uuid = request.params.uuid;
        ...
        request.query.startDate || new Date().toISOString().substring(0, 10);
			...
			var queryParts = {
				columns: request.query.fields || [
					...
					'date_format(rtc_date,\'%W\') as day_of_week',
					'count( distinct t1.person_id) as total',
				],
				table: 'etl.flat_hiv_summary',
				where: [
					't1.location_uuid = ? and date_format(rtc_date,\'%Y-%m\') = date_format(?,\'%Y-%m\')',
					uuid,
					startDate,
				],
				group: ['rtc_date'],
		...
		getClinicMonthlySummary: function getClinicMonthlySummary(
			request,
			callback
		) {
			var uuid = request.params.uuid;
        ...
        request.query.startDate || new Date().toISOString().substring(0, 10);
			...
			var queryParts = {
				columns: request.query.fields || [
					...
					'date_format(encounter_datetime,\'%W\') as day_of_week',
					'count( distinct t1.person_id) as total',
				],
				table: 'etl.flat_hiv_summary',
				where: [
					't1.location_uuid = ? and date_format(encounter_datetime,\'%Y-%m\') = date_format(?,\'%Y-%m\')',
					uuid,
					startDate,
				],
