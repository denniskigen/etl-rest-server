...
module.exports = (function () {
	return {
		...
		resolveLocationUuidsToName: function resolveLocationUuidsToName(
			uuids,
			callback
		) {
			var queryParts = {
			...
			return new Promise(function (resolve, reject) {
				db.reportQueryServer(queryParts, function (results) {
					if (results.error) {
						...
						reportFactory.buildIndicatorsSchema(reportParams, function (
							indicators
						) {
							results.indicatorDefinitions = indicators;
							var resolved = reportFactory.resolveIndicators(
								reportParams.reportName,
								results,
								reportParams.requestIndicators
							);
							resolve(resolved);
						});
					}
		...
		getDataEntryIndicators: function getDataEntryIndicators(
			subType,
			request,
			callback
		) {
		...
		getPatientFlowData: function getPatientFlowData(request, callback) {
			var reportName = 'patient-flow-report';
			var dateStarted =
        request.query.dateStarted || new Date().toISOString().substring(0, 10);
			if (!_.isUndefined(dateStarted)) dateStarted = dateStarted.split('T')[0];
			var locations;
			if (request.query.locations) {
				locations = [];
			...
			db.reportQueryServer(queryParts, function (results) {
				var finalResults = reportFactory.resolveIndicators(reportName, results);
				if (
					request.query.excludePatientList &&
          (request.query.excludePatientList === 'true' ||
            request.query.excludePatientList === true)
				) {
					var removePatientList = function (results) {
		...
		getClinicLabOrdersData: function getClinicLabOrdersData(request, callback) {
			var reportName = 'clinic-lab-orders-report';
			var dateActivated =
        request.query.dateActivated ||
        new Date().toISOString().substring(0, 10);
			if (!_.isUndefined(dateActivated))
				dateActivated = dateActivated.split('T')[0];
			var locations;
			if (request.query.locations) {
				locations = [];
				_.each(request.query.locations.split(','), function (loc) {
		...
		getIndicatorsSchemaWithSections: function getIndicatorsSchemaWithSections(
			request,
			callback
		) {
			var reportName = request.query.report;
			...
			if (reportName === undefined)
				callback(
					Boom.badRequest(
						'report (Report Name) is missing from your request query'
					)
				);
			//build query params
			var queryParams = {
			...
			//retrieve jsin
			reportFactory.buildIndicatorsSchemaWithSections(queryParams, function (
				result
			) {
				var schema = {};
				schema.result = result;
		...
		getIdsByUuidAsyc: function getIdsByUuidAsyc(
			fullTableName,
			idColumnName,
			uuidColumnName,
			arrayOfUuids,
        ...
        request.query.endDate || new Date().toISOString().substring(0, 10);
			...
			var columns =
        'name as location, t1.*, day(encounter_datetime) as day, t3.gender, ' +
        'week(encounter_datetime) as week, month(encounter_datetime) as month, year(encounter_datetime) as year,' +
        'DATE_FORMAT(NOW(), \'%Y\') - DATE_FORMAT(t3.birthdate, \'%Y\') - (DATE_FORMAT(NOW(), \'00-%m-%d\') < ' +
        'DATE_FORMAT(t3.birthdate,\'00-%m-%d\')) AS age, if(arv_start_date is not null, t1.person_id,null) as on_arvs, ' +
        't2.location_id';
		...
		getPatientListReport: function getPatientListReport(requestParams) {
			var requestIndicators = requestParams.indicator;
			var startDate =
        requestParams.startDate || new Date().toISOString().substring(0, 10);
			var endDate =
        requestParams.endDate || new Date().toISOString().substring(0, 10);
			var order = helpers.getSortOrder(requestParams.order);
			var reportName = requestParams.reportName || 'hiv-summary-report';
			var locationIds = requestParams.locations || '';
			var locationUuids = [];
			...
			var queryParts = reportFactory.buildPatientListReportExpression(
				queryParams
			);
			return new Promise(function (resolve, reject) {
				if (!_.isEmpty(queryParts)) {
					...
					reject(
						Boom.badRequest(
							'An error occurred while generating patient list, please check parameter and try again'
						)
					);
				}
			});
