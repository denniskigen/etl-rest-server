		...
		try {
			resolveEnrollmentUuidsToIds(newPatientReferral)
				.then(function (newPateintReferral) {
					validateCreateReferralPayload(newPateintReferral).then(function (
						validationStatus
					) {
							...
							connection
								...
								.then(function (conn) {
									var query = squel
										...
										.set(
											'patient_program_id',
											newPateintReferral.patientProgramId
										)
										.set('provider_id', newPateintReferral.providerId)
										.set(
											'referred_from_location_id',
										...
										.set(
											'notification_status',
											newPateintReferral.notificationStatus
										)
									...
									conn.query(query, {}, function (err, rows, fields) {
										...
										if (err) {
											...
											var resolvedFieldsCheck = checkResolvedReferralFields(
												newPateintReferral
											);
								...
								.catch(function (err) {
									console.log(err);
									reject(
										'Error establishing connection to MySql Server for updated resource'
									);
								});
						}
					});
				...
				.catch(function (error) {
					reject(
						'could not resolve all payload property uuids to their respective ids ',
						error
					);
				});
		} catch (error) {}
...
function updatePatientReferralNotification(
	patientReferralId,
	newPatientReferral
) {
	return new Promise(function (resolve, reject) {
		validateUpdateNotificationPayload(newPatientReferral).then(function (
			validationStatus
		) {
			if (validationStatus.isValid === false) {
				...
				connection
					...
					.catch(function (err) {
						reject(
							'Error establishing connection to MySql Server to update the resource'
						);
					});
			}
		});
			...
			.then(function (conn) {
				...
				conn.query(query, {}, function (err, rows, fields) {
					if (err) {
						console.log(err);
						reject(
							'Error querying to get patient referral details by enrollment server'
						);
					} else {
						if (rows.length > 0) {
							resolve(rows[0]);
	...
	return new Promise(function (resolve, reject) {
		...
		findPatientReferral(
			...
			.catch(function (error) {
				...
				resolve(validationErrors);
				console.log(
					'An error occured while trying to validate patient referral payload',
					error
				);
			});
	}).catch(function (error) {
...
function findPatientReferral(
	encounterId,
	providerId,
	referredToLocationId,
	referredFromLocationId,
	voided
) {
	return new Promise(function (resolve, reject) {
		connection
...
function getEncounterId(encounterUuid) {
	return new Promise(function (resolve, reject) {
		connection
			...
			.catch(function (err) {
				resolve(
					'Error establishing connection to MySql Server to resolve encounter id'
				);
			});
	});
}
function getPatientProgramId(patientProgramUuid) {
	return new Promise(function (resolve, reject) {
		connection
			...
			.catch(function (err) {
				reject(
					'Error establishing connection to MySql Server  to resolve patient program'
				);
			});
	});
}
...
function getWorkFlowState(workFlowStateUuid) {
	return new Promise(function (resolve, reject) {
		connection
			...
			.catch(function (err) {
				reject(
					'Error establishing connection to MySql Server to get workflow state'
				);
			});
	});
}
...
function checkResolvedReferralFields(newPatientReferralPayload) {
	...
	if (
		_.isEmpty(newPatientReferralPayload.providerId) &&
    newPatientReferralPayload.providerId === undefined
	) {
		validationResult.isValid = false;
		validationResult.errors.push({
			field: 'provider',
	...
	if (
		_.isEmpty(newPatientReferralPayload.encounterId) &&
    newPatientReferralPayload.encounterId === undefined
	) {
		validationResult.isValid = true;
	}

	if (
		_.isEmpty(newPatientReferralPayload.referredToLocationId) &&
    newPatientReferralPayload.referredToLocationId === undefined
	) {
		validationResult.isValid = false;
		validationResult.errors.push({
			field: 'referredToLocation',
	...
	if (
		_.isEmpty(newPatientReferralPayload.patientProgramId) &&
    newPatientReferralPayload.patientProgramId === undefined
	) {
		validationResult.isValid = false;
		validationResult.errors.push({
			field: 'patientProgram',
	...
	if (
		_.isEmpty(newPatientReferralPayload.referredFromLocationId) &&
    newPatientReferralPayload.referredFromLocationId === undefined
	) {
		validationResult.isValid = false;
		validationResult.errors.push({
			field: 'referredFromLocation',
