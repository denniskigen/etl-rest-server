...
var App = {
	...
	start: function () {
		let _this = this;
		this.loadQueueData().then(function (data) {
			if (data.length > 0) {
				App.process(data)
					.then(function () {
	...
	process: function (data) {
		...
		let promise = new Promise(function (resolve, reject) {
			data.forEach((arrayItem) => {
				...
				let list = [];
				if (
					program_config[key].incompatibleWith &&
          program_config[key].incompatibleWith.length > 0
				) {
					list = program_config[key].incompatibleWith;
					_.each(list, function (item) {
						patient_programs
							.getPatientProgramEnrollment(person_uuid, list)
							.then(function (rows) {
	...
	deleteProcessed: function (data) {
		...
		var sql =
      'delete from etl.program_registration_queue where person_uuid in (?)';
