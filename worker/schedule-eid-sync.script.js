...
var service = {
  ...
  start: function () {
    ...
    setInterval(function () {
      if (!service.schedulingInProgress) service.scheduleEidSync();
  ...
  scheduleEidSync: function () {
    ...
    if (service.currentTrialCount >= service.maxTrialCount) {
      // bailing out
      console.error(
        'Bailing out...Reached maximum number of trials:',
        service.maxTrialCount
      );
      ...
      if (
        ...
        config.emailNotification.sourceAddress
      ) {
        console.log('*********************************');
        console.log('Sending email notification to maintainers..');
        service
          .sendMail(
            'There was an error sheduling the eid-amrs sync:  ' +
              JSON.stringify(service.errorQueue, null, 4),
            'EID-AMRS sync error',
    ...
    if (service.weekly_sync) {
      ...
      service
        ...
        .catch(function (error) {
          ...
          service.logErrorWhenScheduling(
            'Error scheduling patients with missing VL for date ' +
              startDateVlPending,
            error
          );
          service.sendMail(
            'Error scheduling patients with missing vl' + error,
            'Error Scheduling EID-AMRS Sync For patients with missing VL',
            'ampath-developers@ampath.or.ke'
          );
      ...
      service
        ...
        .then(function (result) {
          ...
          var startDateVlPending = format(
            'yyyy-MM-dd',
            moment(new Date()).subtract(3, 'months').toDate()
          );
          service
            .schedulePatientsWithPendingOrders(startDateVlPending)
            .then(function (results) {
              console.log(
                'Scheduled patients with pending vl orders successfully'
              );
              service.logSuccessfulScheduling(
                'Scheduled patients with pending vl  successfully',
                startDateVlPending
              );
              // console.info('*********************************');
              // console.log('Exiting scheduler...');
              // process.exit(0);
              service
                .rescheduleEidSyncErrors()
                .then(function (res2) {
                  console.log('Scheduled patients in error queue successfully');
                  service.logSuccessfulScheduling(
                    'Scheduled patients in error queue successfully'
                  );
                  console.info('*********************************');
                  console.log('Exiting scheduler...');
                  process.exit(0);
                ...
                .catch(function (err) {
                  console.error(
                    'Error rescheduling patients in error queue',
                    err
                  );
                  service.logErrorWhenScheduling(
                    'Error rescheduling patients in error queue ',
                  ...
                  service.sendMail(
                    'Error rescheduling patients in error queue' + err,
                    'Error Scheduling EID-AMRS Sync For patients in error queue',
                    'ampath-developers@ampath.or.ke'
                  );
            ...
            .catch(function (error) {
              console.error(
                'Error scheduling patients with pending vl orders',
                error
              );
              service.logErrorWhenScheduling(
                'Error scheduling patients with pending VL order for date ' +
                  startDateVlPending,
                error
              );
              service.sendMail(
                'Error scheduling patients with pending vl orders' + error,
                'Error Scheduling EID-AMRS Sync For patients with pending VL',
                'ampath-developers@ampath.or.ke'
              );
  ...
  fetchAllPatientResults: function (configObj, options) {
    let promises = [
      service.fetchAllViralLoad(configObj, options),
      service.fetchAllDNAPCR(configObj, options),
  ...
  scheduleQueue: function () {
    ...
    return service
      .fetchAllPatientResults(configObj, options)
      .then((identifiers) => {
  ...
  scheduleEidSyncPerServerPerType: function (queueItem) {
    ...
    return new Promise(function (resolve, reject) {
      console.log(
        'Fetching results from eid...',
        queueItem.host,
        queueItem.type
      );
      service
        .getEidQueryPromise(queueItem)
        .then(function (result) {
          ...
          if (
            ...
            service
              .insertPatientsWithEidResultsIntoSyncQueue(
                result.patientIdentifiers
              )
              ...
              .catch(function (error) {
                console.error(
                  'An error occured while inserting into eid sync queue',
                  error
                );
                reject(error);
              });
        ...
        .catch(function (error) {
          console.error(
            'Fetching results from eid done with an error: ',
            error
          );
          reject(error);
        });
  ...
  getEidQueryPromise: function (queueItem) {
    switch (queueItem.type) {
      case 'vl':
        return eidService.getPatientIdentifiersWithEidResultsByType(
          queueItem.host,
          queueItem.apiKey,
          queueItem.startDate,
          queueItem.endDate,
      ...
      case 'dnapcr':
        return eidService.getPatientIdentifiersWithEidResultsByType(
          ...
          queueItem.startDate,
          queueItem.endDate,
          eidService.getEidDnaPcrResults
        );
        break;
      default:
  ...
  schedulePatientsWithMissingVlPastOneYear: function () {
    var sql =
      'replace into  etl.eid_sync_queue(person_uuid) ' +
  ...
  rescheduleEidSyncErrors: function () {
    return new Promise(function (resolve, reject) {
      service
        .insertEidQueueErrorsIntoEidSyncQueue()
