		...
		es
			.mapSync(function (line) {
				...
				try {
					if (line && line !== '') {
						console.log(
							'Enrolling a patient into Standard HIV Care Program: ',
							line
						);
              ...
              config.eidSyncCredentials.password;
						...
						curl.request(options, function (err, parts) {
							if (err || (parts && JSON.parse(parts).error)) {
								console.log(
									'error enrolling a patient into program: ' + line,
									err || JSON.parse(parts).error
								);
								fs.appendFileSync(error_file, line + '\r\n');
							} else {
								console.log(
									'Enrolled Patient Into Standard HIV Treatment: ' + line
								);
							}
