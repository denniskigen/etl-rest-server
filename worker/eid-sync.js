...
var Sync = {
	...
	start: function () {
		...
		setInterval(function () {
			if (!Sync.processing) Sync.process();
		}, Sync.timeout);
	},
	...
	process: function () {
		...
		if (moment().isBefore(Sync.nextSyncDateTime)) {
			console.log(
				'Sync paused and will resume ' + Sync.nextSyncDateTime.fromNow()
			);
		...
		this.loadDbRecords().then(function (data) {
			if (data.length > 0) {
		...
		return new Promise(function (resolve, reject) {
			curl.request(options, function (err, parts) {
				if (err) {
					...
					console.log(
						'error while syncing ' + patientUuId + '. Logging error.'
					);
	...
	logError: function (patientUuId, error) {
		var sql =
      'INSERT INTO etl.eid_sync_queue_errors(person_uuid, error, date_created)' +
      ' VALUES(\'' +
      patientUuId +
      '\',\'' +
      error +
