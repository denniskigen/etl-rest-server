...
export class LabSyncService {
  ...
  syncLabsByPatientUuid(patientUuid, labLocation, delay) {
    ...
    return this.getLabSyncLog(patientUuid)
      .then((result) => {
        var patientHasEverBeenSynced = false;
        if (result.result.length > 0) patientHasEverBeenSynced = true;
        if (patientHasEverBeenSynced) {
          if (
            result.result[0]['status'] === 0 &&
            (result.result.length === 1 || result.result[1]['status'] === 0) &&
            result.result[0]['TIMESTAMPDIFF(HOUR,date_updated,now())'] < 6
          ) {
            console.log('Patient synced..');
            return obsService
              .getPatientTodaysTestObsByPatientUuId(patientUuid)
      ...
      .then(function (response) {
        ...
        return Promise.all([
          ...
          .then((allResults) => {
            ...
            labResultsPromises.push(
              that
                .processViralLoadResults(
                  viralLoadResults.data || [],
                  patientUuid
                )
                .then((processedObs) => {
            ...
            labResultsPromises.push(
              that
                .processDNAPCRResults(dnaPCR.data || [], patientUuid)
                .then((processedObs) => {
                  const eidCompareOperator = new EidCompareOperator();
                  ...
                  return eidCompareOperator.getAllResults(
                    labDNAPCRObs,
                    amrsObs
                  );
                })
            ...
            labResultsPromises.push(
              that
                .processCD4Results(cd4Results.data || [], patientUuid)
                .then((processedObs) => {
                  const eidCompareOperator = new EidCompareOperator();
            ...
            return Promise.all(labResultsPromises)
              .then((obs) => {
                const flatten = (arr) =>
                  arr.reduce(
                ...
                let combinedMissing = that.combineObs(
                  'missingResults',
                  flattenResult
                );
                let combinedconflicting = that.combineObs(
                  'conflictingResults',
                  flattenResult
                );
                ...
                return Promise.all(that.combineObsPostPromises(combinedMissing))
                  .then((savedObs) => {
                    fields[0].status = 0;
  ...
  processViralLoadResults(result, patientUuid) {
    ...
    return Promise.all([
      vlAdapter.getLabResults(),
      obsService.getPatientAllTestObsByPatientUuId(patientUuid),
    ]);
  }
