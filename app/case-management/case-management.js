	...
	getCaseManagementData: (params, callback) => {
        ...
        'AS patients_due_for_vl ';
          ...
          '\' ';
			...
			if (
				params.minFollowupPeriod != null ||
        params.maxFollowupPeriod != null
			) {
				followupDateRange = convertDaysToDate(
					params.minFollowupPeriod,
					params.maxFollowupPeriod
				);
				where =
          where +
          ...
          '\') ';
			...
			if (params.phoneFollowUpStartDate != null) {
				where =
          where +
          'and next_phone_appointment =  DATE(\'' +
          ...
          '\') ';
			...
			if (params.isNewlyEnrolled == 1) {
				where =
          where +
          'and TIMESTAMPDIFF(DAY,DATE(t1.enrollment_date),curdate()) <= 30 ';
          ...
          'and TIMESTAMPDIFF(DAY,DATE(t1.enrollment_date),curdate()) > 30 ';
			...
			if (params.caseManagerUserId != null) {
				where =
          where +
          'and case_manager_user_id in (' +
          params.caseManagerUserId +
          ')';
			}
			sql =
        'select ' +
        ...
        'WHERE ( ' +
        where +
        ' and t1.transfer_out is null and transfer_transfer_out_bncd is null ) group by t1.person_id order by t1.vl_1 desc';
			queryParts = {
				sql: sql,
...
function updateCaseManager(params, update) {
	return new Promise((resolve, reject) => {
		...
		if (update == true) {
			sql =
        'update etl.flat_case_manager set case_manager_user_id = \'' +
        params.user_id +
        ...
        '\' ;';
			...
			sql =
        'update etl.flat_case_manager set case_manager_user_id = NULL, case_manager_user_name = NULL, case_manager_name = NULL  where uuid = \'' +
        params.patient_uuid +
        '\' ;';
		}
		queryParts = {
...
function convertDaysToDate(minDays, maxDays) {
	return [
		moment().subtract(minDays, 'days').format('YYYY-MM-DD'),
		moment().subtract(maxDays, 'days').format('YYYY-MM-DD'),
	];
}
