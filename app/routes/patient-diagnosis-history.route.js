        ...
        getPatientOncologyDiagnosis(requestParams)
          .then((data) => {
            ...
            _.each(diagnosisData.result, (summary) => {
              summary.diagnosis = `${helpers.getConceptName(
                summary.cancer_type
              )}${
                summary.cancer_subtype
                  ? ` - ${helpers.getConceptName(summary.cancer_subtype)}`
                  : ''
              }`;
              summary.diagnosis_date = helpers.filterDate(
                summary.diagnosis_date
              );
            });
            diagnosisData.result = _.filter(
              diagnosisData.result,
              (summary) => !_.isNull(summary.cancer_type)
            );
            reply(diagnosisData);
          })
          .catch((error) => {
