...
import {
  generateMedsDataSet,
  getOncMeds,
} from '../../service/oncology/patient-oncology-summary-service';
                  ...
                  _.each(summary.drugs, function (data) {
                    ...
                    _.each(data, function (a) {
                      if (a.concept_id === 9918) {
                        oncMed.cur_onc_meds = helpers.getConceptName(
                          a.value_coded
                        );
                      } else if (a.concept_id === 1899) {
                        oncMed.cur_onc_meds_dose = a.value_numeric;
                      } else if (a.concept_id === 1896) {
                        oncMed.cur_onc_meds_frequency = helpers.getConceptName(
                          a.value_coded
                        );
                      } else if (a.concept_id === 7463) {
                        oncMed.cur_onc_meds_route = helpers.getConceptName(
                          a.value_coded
                        );
                      }
                      if (planAndDate.length > 0) {
                        oncMed.meds_start_date = helpers.filterDate(
                          planAndDate[0].obs_datetime
                        );
                        oncMed.chemotherapy_plan = helpers.getConceptName(
                          planAndDate[0].value_coded
                        );
                      }
                    });
                    if (oncMed !== '') {
