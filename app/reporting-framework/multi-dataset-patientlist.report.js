...
export class MultiDatasetPatientlistReport extends MultiDatasetReport {
        ...
        .then((results) => {
          for (let i = 0; i < results.length; i++) {
            if (
              ...
              results[i].results.results.results
            ) {
              let datta = this.interprateCurrentArvMeds(
                results[i].results.results.results
              );
  ...
  interprateCurrentArvMeds(result) {
    ...
    result.forEach((element) => {
      ...
      (dataObject['gender'] = element.gender),
        ...
        (dataObject['cur_arv_meds'] = helpers.getARVNames(
          element.cur_arv_meds
        )),
        (dataObject['enrollment_date'] = element.enrollment_date),
        (dataObject['arv_first_regimen_start_date'] =
          element.arv_first_regimen_start_date),
        (dataObject['cur_regimen_arv_start_date'] =
          element.cur_regimen_arv_start_date),
        ...
        (dataObject['age_rstarted_modern_contraceptionange'] =
          element.started_modern_contraception),
        (dataObject['on_modern_contraception'] =
          element.on_modern_contraception),
  ...
  runSingleReport(reportObject, additionalParams) {
    if (additionalParams && additionalParams.type === 'patient-list') {
      return reportObject.generatePatientListReport(
        additionalParams.indicators
      );
    }
