    ...
    return {
        ...
        getConceptName: function getConceptName(code) {
            ...
            var concepts = {
                ...
                9626: "HORMONE REPLACEMENT THERAPY",
                10038: "BIOLOGIC THERAPY",
                10232: "TARGETED THERAPY",
...
module.exports = (function () {
	return {
		getReportParams: function getReportParams(
			reportName,
			whereClause,
			queryParams
		) {
			...
			queryParams.endDate = (
				queryParams.endDate || new Date().toISOString().substring(0, 10)
			).split('T')[0];
			queryParams.referenceDate = (
				queryParams.referenceDate || new Date().toISOString().substring(0, 10)
			).split('T')[0];
        ...
        str === '-1'
			...
			var arvs = {
				...
				6180: { mapped_to_ids: '791;802', name: 'EMTRICITABINE AND TENOFOVIR' },
				6467: {
					mapped_to_ids: '628;631;633',
					name: 'NEVIRAPINE LAMIVUDINE AND ZIDOVUDINE',
				},
				6679: { mapped_to_ids: '628;814', name: 'ABACAVIR AND LAMIVUDINE' },
				6964: {
					mapped_to_ids: '628;633;802',
					name: 'TENOFOVIR AND LAMIVUDINE AND EFAVIRENZ',
				},
				6965: { mapped_to_ids: '625;628', name: 'LAMIVUDINE AND STAVUDINE' },
				9435: { mapped_to_ids: '9435', name: 'EVIPLERA' },
				9759: { mapped_to_ids: '9759', name: 'DOLUTEGRAVIR' },
...
=======
		...
		transformMedicalRefillToClinical: function transformMedicalRefillToClinical(
			hivSummaries
		) {
			let transfromedArray = [];
			_.each(hivSummaries, (hivSummary) => {
		...
		resolvedLabOrderErrors: function resolvedLabOrderErrors(
			vlerror,
			cd4eror,
			pcrerror
		) {
			...
			if (vlerror === 1) message = 'Error processing Viral Load';
			if (cd4eror === 1) {
				if (message !== '') message = message + ', ' + 'Error processing cd4';
				else message = 'Error processing cd4';
			}

			if (pcrerror === 1) {
				if (message !== '')
					message = message + ', ' + 'Error processing hiv dna pcr';
				else message = 'Error processing hiv dna pcr';
			}

			return message;
		},

		buildWhereClauseForDataEntryIndicators: function buildWhereClauseForDataEntryIndicators(
			queryParams,
			where
		) {
			if (queryParams.locations) {
		...
		buildWhereParamsDataEntryIndicators: function buildWhereParamsDataEntryIndicators(
			queryParams,
			where
		) {
			if (queryParams.locations) {
